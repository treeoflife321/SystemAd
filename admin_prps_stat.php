...
$purpose = ["Study", "Research", "Printing", "Clearance", "Borrow Book(s)", "Return Book(s)"];
$purpose_count = array_fill_keys($purpose, 0);

$query = "SELECT purpose, COUNT(*) as count FROM chkin WHERE archived = ' ' GROUP BY purpose";
$result = $mysqli->query($query);

if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $level = $row['purpose'];
        if (array_key_exists($level, $purpose_count)) {
            $purpose_count[$level] += $row['count'];
        }
    }
}

$total_students = array_sum($purpose_count);
?>
...
// Query to count students per purpose, with optional date filter
$query = "SELECT purpose, COUNT(*) as count FROM chkin WHERE archived = ' '";
...
// Process result to populate Purpose counts
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $level = $row['purpose'];
        if (array_key_exists($level, $purpose_count)) {
...
$header_text = "User Purpose Distribution";
if ($start_date && $end_date) {
...
<body class="bg">
    ...
    <div class="content">
        <nav class="secondary-navbar">
            ...
            <a href="admin_prps_stat.php<?php if (isset($aid)) echo '?aid=' . $aid; ?>" class="secondary-navbar-item active">User Purpose Graph</a>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Define a color for each purpose
    var purposeColors = {
        "Study": 'rgba(255, 87, 104, 0.8)',  // Soft Red
        "Research": 'rgba(75, 192, 192, 0.8)', // Aqua Blue
        "Printing": 'rgba(255, 205, 86, 0.8)', // Bright Yellow
        "Clearance": 'rgba(102, 205, 170, 0.8)', // Sea Green
        "Borrow Book(s)": 'rgba(153, 102, 255, 0.8)',  // Soft Purple
        "Return Book(s)": 'rgba(255, 159, 64, 0.8)'  // Vibrant Orange
    };

    // Prepare the labels (purposes) and data (student counts)
    var labels = <?php echo json_encode(array_keys($purpose_count)); ?>;
    var data = <?php echo json_encode(array_values($purpose_count)); ?>;

    // Prepare the background colors for each purpose
    var backgroundColors = labels.map(function(purpose) {
        return purposeColors[purpose] || 'rgba(0, 0, 0, 0.5)';  // Fallback color
    });

    // Prepare the border colors for each purpose
    var borderColors = labels.map(function(purpose) {
        return purposeColors[purpose]?.replace('0.5', '1') || 'rgba(0, 0, 0, 1)';  // Darken the border color
    });
    ...
    var yearLevelChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'User Purpose Distribution'
                }
            }
        }
    });
