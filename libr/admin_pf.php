...
function updateAdminData($mysqli, $aid) {
    ...
    if(isset($_POST['name']) && isset($_POST['contact']) && isset($_POST['username'])) {
        ...
        $updateQuery = "UPDATE libr SET name = ?, contact = ?, username = ?";
        $params = array("s", "s", "s");
        ...
        if(!empty($_POST['password'])) {
            ...
            $checkPasswordQuery = "SELECT password FROM libr WHERE aid = ?";
...
if(isset($_GET['aid'])) {
    ...
    $query = "SELECT name, contact, username FROM libr WHERE aid = ?";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("i", $aid);
    $stmt->execute();
    $result = $stmt->get_result();
...
<a href="admin_dash.php<?php if(isset($aid)) echo '?aid=' . $aid; ?>"><button class="cancel-btn" type="button">X</button></a>

    <center><h1>Librarian Profile</h1></center>
